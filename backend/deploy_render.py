...
def get_api_key() -> str:
    ...
    if not key:
        ...
        print("Please set RENDER_API_KEY in your environment or backend/.env file.")
        sys.exit(1)
    return key
...
def main():
    ...
    if res.status_code != 200:
        print(f"❌ Failed to connect to Render API ({res.status_code}): {res.text}")
        sys.exit(1)
    ...
    if backend_service:
        ...
        print("📦 Creating new Web Service 'fileforge-backend' on Render...")
        payload = {
            "type": "web_service",
        ...
        if res.status_code not in (200, 201):
            print(f"❌ Service creation failed ({res.status_code}): {res.text}")
            sys.exit(1)
    ...
    if res.status_code == 200:
        ...
        print(f"⚠️ Warning updating env vars ({res.status_code}): {res.text}")
    ...
    if res.status_code in (200, 201):
        ...
        print(f"⚠️ Deploy trigger note ({res.status_code}): {res.text}")
        deploy_id = None
