...
import { FileImage, FileText, Eye, Pencil, Sliders } from "lucide-react";
import CanvasEditor from "./CanvasEditor";

interface FilePreviewProps {
  file: File | null;
  onFileEdited?: (editedFile: File) => void;
}

export default function FilePreview({ file, onFileEdited }: FilePreviewProps) {
  ...
  const handleCanvasSave = (editedFile: File) => {
    setShowCanvasEditor(false);
    if (onFileEdited) {
      onFileEdited(editedFile);
    }
  };
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          {isImage && (
            <button
              ...
              className="px-3 py-1 rounded-lg bg-brand-500/20 hover:bg-brand-500/30 border border-brand-500/30 text-brand-300 transition text-xs font-medium flex items-center gap-1.5"
            >
              <Pencil className="w-3.5 h-3.5" /> Open Canvas Editor
            </button>
          )}
        </div>
      ...
      {/* Interactive Canvas Editor Modal */}
      {showCanvasEditor && file && (
        <CanvasEditor
          file={file}
          onSave={handleCanvasSave}
          onCancel={() => setShowCanvasEditor(false)}
        />
      )}
    </>
  );
